...
cur.execute('''CREATE TABLE IF NOT EXISTS Pages
    (id INTEGER PRIMARY KEY, url TEXT UNIQUE, html TEXT, size INTEGER,
     code INTEGER, content_type TEXT, old_rank REAL, new_rank REAL)''')
...
while True:
    ...
    try:
        ...
        actualurl = document.geturl()

        html = document.read()
        info = document.info()

        cur.execute('UPDATE Pages SET code=?, size=?, content_type=? WHERE url=?',
                (document.getcode(), len(html), info.get_content_type(), url) )
        ...
        print("HTTPError")
        print(exc)
        cur.execute('UPDATE Pages SET code=? WHERE url=?', (exc.code, url) )
        conn.commit()
        continue
    except Exception as exc:
        print("Unable to retrieve or parse page")
        print(exc)
        cur.execute('UPDATE Pages SET code=-1 WHERE url=?', (url, ) )
        conn.commit()
        continue
